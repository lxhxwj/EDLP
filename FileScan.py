...
def typeList():
	print(u'获取文件格式十六进制码表……')
	return {
		"68746D6C3E":'html',
		"574C46534F4656356E00":'WLFS',
...
def filescanner(path):
	if type(path) != type('a'):#判断是否为字符串
		print(u'抱歉，你输入的不是一个字符串路径！')
	elif path.strip() == '':#将两头的空格移除
		print(u'输入的路径为空！')
	elif not os.path.exists(path):
		print(u'输入的路径不存在！')
	elif os.path.isfile(path):
		print(u'输入的路径指向的是文件，验证文件类型……')
		if path.rfind('.') > 0:
			print(u'文件名:',os.path.split(path)[1])
		else:
			print(u'文件名中没有找到格式')
		path = filetype(path)
		print(u'解析文件判断格式：'+path)
	elif os.path.isdir(path):
		print(u'输入的路径指向的是目录，开始遍历文件')
...
if __name__ == '__main__':
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "hf:v", ["help", "version","fpath"])
